...
if (param && param.trim() == 'init') {
    if (fs.existsSync(configFilePath)) {
        console.log('config file already exists, please edit config file: ' + path.resolve(configFilePath));
        process.exit(0);
    }
    const config = {
        language: 'zh',
...
// 检查目录是否存在，如果不存在则创建它
    ...
    console.log('init success, please edit config file: ' + path.resolve(configFilePath));
    process.exit(0);
}
...
export const gen = async () => {
    ...
    const diff = await child_process.execSync('git diff').toString().substring(0, 5000);
